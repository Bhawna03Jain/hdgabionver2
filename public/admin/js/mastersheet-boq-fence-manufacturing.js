$(document).ready(function () {
    ...
    const editButton = document.getElementById(
        "edit-Fence-Manufacturing-Button"
    );
    const updateButton = document.getElementById(
        "update-Fence-Manufacturing-Button"
    );
    const deleteButton = document.getElementById(
        "delete-Fence-Manufacturing-Button"
    );
        ...
        if (codeValue !== "" && boqConfigId) {
            try {
                const response = await $.ajax({
                    url: `/admin/mastersheet/boq/fence/check-code/manufacturing`,
                    method: "POST",
                    headers: {
    ...
    $("#addRowButton").click(function () {
        ...
        $.ajax({
            url: "/admin/mastersheet/boq/fence/get-last-id/manufacturing", // Endpoint to get the last ID
            method: "GET",
            success: function (response) {
                ...
                newRow.html(`
                        <td><input type="text" name="extra_manufacturing[${newRowId}][code]" class="form-control manufacturing-input" required ${readonlyAttribute}></td>
                        <td><input type="text" name="extra_manufacturing[${newRowId}][name]" class="form-control manufacturing-input" required ${readonlyAttribute}></td>
                        <td><input type="number" name="extra_manufacturing[${newRowId}][cost_per_unit]" class="form-control manufacturing-input" required ${readonlyAttribute}></td>
            ...
            if (!isValid) {
                ...
                if (confirm("Do you want to update the data?")) {
                    ajaxFormSubmit(
                        ...
                        "/admin/mastersheet/boq/fence/update/manufacturing", // URL for the store action
                        "post",
                        "Manufacturing Detail For Fence has been Configured Successfully." // Success message
                    );
                } else {
                    return false;
                }
